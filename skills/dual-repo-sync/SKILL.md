...
```bash
git diff --name-only public@{1}..public | Select-String -Pattern "^progress_docs|^MEMORY|^CLAUDE|^.vscode|^.python-version|.code-workspace"
```

**If ANY output appears:** Files have leaked. Run cleanup:
```bash
git rm --cached -r progress_docs/ .vscode/ 2>$null
